    ...
    try { // 读输入找host和port
      ...
      if (headtext != "") {
        ...
        if (Proxy.buffer.containsKey(headtext)) {
          ...
          if(Proxy.buffer.get(headtext).getDate()!=null) {
...
//            }
//            int port = head.getPort();
            proxySocket = new Socket("www.sdbeta.com", 80);
            if (proxySocket != null) {
              proxyOutput = proxySocket.getOutputStream();
              proxyInput = proxySocket.getInputStream();

              System.out.println("假装准备开始请求" + host + ":" + port);
              String fishhead="GET http://www.sdbeta.com/ HTTP/1.1\r\n" + 
                  "Host: www.sdbeta.com\r\n" + 
              ...
              proxyOutput.write(fishhead.getBytes());
              System.out.println("请求已发出");
